...
//---------------------------------------------------------------------------------------
//Tags not needing any processing. Just replace tag by the corresponding
//open/close out tag.
    ...
    if (!IsEmpty())
    {
        if (m_fIsSingle)
            wxLogMessage(wxString::Format(_T("Tag='%s', level=%d, single (%d, %d), IsPH=%s, Replz='%s'"),
                         m_pParent->GetContent().Mid(m_nStartOpen+1, m_nNameLength).wc_str(),
                         m_nLevel,
                         m_nStartOpen, m_nStartClose,
                         (IsPlaceholder() ? _T("true") : _T("false")),
                         m_sOpenReplacement.wc_str()
                        ));
        else
        {
            wxLogMessage(wxString::Format(_T("Tag='%s', level=%d, open (%d, %d), close(%d, %d), IsPH=%s, OpenReplz='%s', CloseReplz='%s'"),
                         ...
                         (IsPlaceholder() ? _T("true") : _T("false")),
                         m_sOpenReplacement.wc_str(), m_sCloseReplacement.wc_str()
                        ));
        }
    }
...
void lmElement::ClosePlaceholder(int nTag, lmContentStorage* pCS)
{
    ...
    //    pCS->AddElementClose(wxString::Format(_T("</%s-%d>"), sName.wc_str(), nTag), GetCloseTag());
    //else
    //    pCS->AddElementClose(wxString::Format(_T("</%s>"), sName.wc_str()));
    if (lmTags::is_placeholder_tag(sName) || lmTags::is_supress_tag(sName))
...
void lmElement::OpenPlaceholder(int nTag, lmContentStorage* pCS)
{
    ...
    //if (lmTags::is_placeholder_tag(sName) || lmTags::is_supress_tag(sName))
    //    pCS->AddElementOpen(wxString::Format(_T("<%s-%d>"), sName.wc_str(), nTag), GetOpenTag());
    //else
    //    pCS->AddElementOpen(wxString::Format(_T("<%s>"), sName.wc_str()));
    if (lmTags::is_placeholder_tag(sName) || lmTags::is_supress_tag(sName))
...
void lmElement::SinglePlaceholder(int nTag, lmContentStorage* pCS)
{
    ...
    if (lmTags::is_supress_tag(sName))
        pCS->AddElementSingle(wxString::Format(_T("<%s-%d />"), sName.wc_str(), nTag), GetFullContent() );
    else if (lmTags::is_placeholder_tag(sName))
        pCS->AddElementSingle(wxString::Format(_T("<%s-%d />"), sName.wc_str(), nTag), GetOpenTag() );
    else
        pCS->AddElementSingle(wxString::Format(_T("<%s />"), sName.wc_str()));
}
                                   ...
                                   const wxString& sCharCode, int nOptions)
{
    ...
    LogMessage(wxString::Format("Loading file %s", sFilename.wc_str()));
    wxXml2Document oDoc;
    wxString sError;
    if (!oDoc.Load(sFilename, &sError)) {
        LogMessage(wxString::Format("*** Error parsing file %s\nError:%s",
                   sFilename.wc_str(), sError.wc_str()));
        return false;
    }
    ...
    if (m_fOnlyLangFile)
 {
        if (!StartLangFile( sFilename )) {
            LogMessage(wxString::Format("*** Error: Lang file '%s' can not be created.",
                                        sFilename.wc_str()));
    ...
    {
        LogMessage(wxString::Format("*** Error: toc file '%s' can not be created.",
                                    sFilename.wc_str()));
        oRoot.DestroyIfUnlinked();
        oDoc.DestroyIfUnlinked();
        return false;        //error
    ...
    if (m_fGenerateLmb)
    {
        ...
        {
            LogMessage(wxString::Format("*** Error: LMB file '%s' can not be created.",
                                        sFilename.wc_str()));
            oRoot.DestroyIfUnlinked();
            oDoc.DestroyIfUnlinked();
            return false;        //error
    ...
    {
        LogMessage(
            wxString::Format("*** Error. First tag is neither <book> nor <article> but <%s>",
            oRoot.GetName().wc_str() ));
        oRoot.DestroyIfUnlinked();
        oDoc.DestroyIfUnlinked();
        return false;
    }
                                       ...
                                       bool fInTextContainerTag)
{
    ...
    if (m_fLogTree) wxLogMessage(_T("[ProcessChildren] nodeType=%d. Name='%s', content='%s'"),
                                 oNode.GetType(), oNode.GetName().wc_str(), oNode.GetContent().wc_str() );

    if (oNode.GetType() == wxXML2_TEXT_NODE)
    {
        ...
        if (!tmp.IsEmpty())
        {
//            wxLogMessage(_T("[ProcessChildren] Text node. Content='%s'"),
//                         sContent.wc_str() );
    ...
    else if (oNode.GetType() == wxXML2_ELEMENT_NODE)
    {
    ...
    else if (oNode.GetType() == wxXML2_ENTITY_DECL)
    {
...
//                     oNode.GetName().wc_str(), pNode->GetSystemID().wc_str() );
        ...
        if (m_fExtEntity &&  m_sExtEntityName == oNode.GetName())
        {
            ...
            wxXml2Document oDoc;
            wxString sError;
            LogMessage(wxString::Format("Processing %s.",
                                        oFN.GetFullName().wc_str()));
            if (!oDoc.Load(oFN.GetFullPath(), &sError)) {
                LogMessage(wxString::Format("*** Error parsing file %s\nError:%s",
                                            oFN.GetFullPath().wc_str(), sError.wc_str()));
                return true;    //error
            }
...
//        else
//        {
//            wxLogMessage(_T("[ProcessChildren] External entity (17) ignored. Name='%s', SystemID='%s'"),
//                         oNode.GetName().wc_str(), pNode->GetSystemID().wc_str() );
//        }
    }
    else if (oNode.GetType() == wxXML2_ENTITY_REF_NODE) {
...
//        wxLogMessage(_T("[ProcessChildren] Entity Ref node, Name='%s'"),
//                        oNode.GetName().wc_str() );
        ...
        // ignore it
//        wxLogMessage(_T("[ProcessChildren] NodeType=%d ignored, Name='%s'"),
//                        oNode.GetType(), oNode.GetName().wc_str() );
    }
...
//    LogMessage(wxString::Format("Replacing tag %s. Open: %s",
//                                sTag.wc_str(), tReplz.sOpen.wc_str()));
...
//    LogMessage(wxString::Format(". Close: %s", tReplz.sClose.wc_str()));
...
bool lmEbookProcessor::ProcessTag(const wxXml2Node& oNode, lmContentStorage* pResult)
{
    ...
    if (oNode.GetType() != wxXML2_ELEMENT_NODE) return false;

    bool fError = false;
    wxString sElement = oNode.GetName();
    //wxString spaces(wxT(' '), m_xx);
    //wxLogMessage(spaces + _T("[ProcessTag] - lmElement [%s]"), sElement.wc_str());
    ...
    else if (sElement == _T("musicfont")) {
        fError = MusicfontTag(oNode, pResult);
    }
    ...
    //unrecognized tag
        ...
        LogMessage(wxString::Format("*** Error: Found tag <%s>. No treatment defined.",
                                    sElement.wc_str()));
        fError = true;
    }
...
//---------------------------------------------------------------------------------------
bool lmEbookProcessor::MusicfontTag(const wxXml2Node& oNode, lmContentStorage* pResult)
{
    // openning tag
    pResult->Add(_T("<musicfont>"));

    //process tag's children and replace labels by theirs Unicode chars
    lmContentStorage cs;
    bool fError = GetTagContent(oNode, &cs);
    wxString sEntities = cs.GetContent();
    //wxString cutCommon = L"\xE08B";
    sEntities.Replace("$cutCommon;", L"\xE08B");

    pResult->Add(sEntities);
...
bool lmEbookProcessor::StartTocFile(wxString sFilename)
{
    ...
    if (!m_pTocFile->IsOpened()) {
        LogMessage(wxString::Format("*** Error: File %s can not be created",
                                    oTOC.GetFullName().wc_str()));
        return false;        //error
    }
...
bool lmEbookProcessor::start_content_file(const wxString& sFilename)
{
    ...
    if (m_fGenerateLmb)
    {
        ...
        //wxLogMessage(_T("[lmEbookProcessor::start_content_file] starting file '%s'"), name.wc_str());
    }
...
//        wxLogMessage(sText);
        ...
        else    //k_format_lmd
        {
            ...
            trans.Replace(_T("<musicfont>"), _T(" <txt style='eBook_music'>"));
            trans.Replace(_T("</musicfont>"), _T("</txt>"));
        }
...
{
    wxString lang = "<lenmusdoc vers='0.0' language='" + m_sLangCode + "'>\n";
    write_to_content_file(lang);
    ...
    write_to_content_file(
        _T("   <defineStyle>\n")
        _T("      <name>eBook_music</name>\n")
        _T("      <font-name>Bravura</font-name>\n")
        _T("      <font-size>12pt</font-size>\n")
        _T("      <color>#000000</color>\n")
        _T("   </defineStyle>\n")
    );
...
void lmEbookProcessor::TerminateLmbFile()
{
    ...
    {
        LogMessage(wxString::Format("*** Error: File %s could not be deleted",
                                    m_sTocFilename.wc_str()));
    }
...
void lmEbookProcessor::CopyToLmb(wxString sFilename, const wxString& subfolder)
{
    ...
    if (!inFile.IsOk())
    {
        LogMessage(wxString::Format("*** Error: File %s can not be merged into LMB",
                                    sFilename.wc_str()));
        return;
    }
...
bool lmEbookProcessor::StartLangFile(wxString sFilename)
{
    ...
    LogMessage(wxString::Format("Creating file '%s'",
                                oFDest.GetFullPath().wc_str()));
...
#define STEP            4
    ...
    if (oNode.GetType() == wxXML2_TEXT_NODE ||
        oNode.GetType() == wxXML2_COMMENT_NODE ||
        oNode.GetType() == wxXML2_CDATA_SECTION_NODE) {
...
//---------------------------------------------------------------------------------------
void lmEbookProcessor::LogMessage(const wxString& sMsg)
{
    if (m_pLog)
    {
...
//---------------------------------------------------------------------------------------
void lmEbookProcessor::LogError(const wxString& sMsg)
{
    if (m_pLog)
        m_pLog->AppendText(sMsg);
    else
